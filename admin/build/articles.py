...
ARTICLES = [
    ...
    ("a-ledger-and-a-record", "v0.9.0", "22 September 2026",
     "Two more cases: this site's own session as a ledger, and three surfaces of one product "
     "over a record that contains secrets",
     "The cost walkthrough said no case had run its prompts. The first case here is that "
     "case, on the one shape whose grant was measured, with a ledger counted from the "
     "repository and the workflow log. The second is a deployer with one assistant on three "
     "surfaces and one rule: reading the past is on demand."),
    ("context-on-what-matters", "v0.10.0", "22 September 2026",
     "The desktop walkthrough: on your own machine, host means your machine, and the mandate "
     "is a map of what matters before it is a list of rules",
     "The third walkthrough in the same four steps, because the workflow is meant to always "
     "be the same. On a machine the published shape's character is that reading files, "
     "changing them and running commands each sit at a switch the account can flip. The "
     "concept the section is built on is the deployer's: what is being given to the agent "
     "is context on what is important and what is not."),
]
...
def _v090(up):
    def s(name, alt, caption):
        return shot(up, name, alt, caption, "v0.9.0", CAPTURED_V080)

    return [
        ("h2", "The case the cost walkthrough said did not exist yet"),
        ("p", "The article for v0.8.0 ended on a list of what that release did not settle, "
              "and the fourth item was that no case ran the cost prompts. **The obvious "
              "candidate was the session writing the sentence.** It runs on the one shape "
              "this site holds whose grant was measured by the thing being profiled, its "
              "commits and pushes are in the repository and the code host's workflow log, "
              "and the deployer's instructions are nine messages anybody can count."),
        s("v090-cases-index",
          "The cases index at v0.9.0 with three cards: the beta user, the site's own session "
          "as a ledger, and three surfaces over one record",
          "Three cases. The second card is the site counting itself, and its foot says with "
          "a ledger where the others say no grant measured."),

        ("h2", "A ledger where every line says where its number came from"),
        ("p", "Seventeen lines. Commits, pushes, pipeline runs, files changed split into "
              "written by hand, copied in, captured and generated, fetches, subagents, "
              "questions asked of the deployer, things handed over, reviews requested, "
              "commands the harness blocked, tokens, and the deployer's own time. **Each line "
              "carries one of five sources: repository, platform, self, estimate, or cannot "
              "see**, and the gate refuses any other word. An estimate carries an asterisk. "
              "A cannot see line carries no number at all, and the gate refuses one that "
              "does."),
        s("v090-ledger",
          "The ledger table on the session case: what was spent, how many, counted from, "
          "and a note, with commits from the repository and tokens marked cannot see",
          "The top of the ledger. Thirteen commits, ten pipeline runs, 523 files of which 20 "
          "were written by hand and 418 were generated, and the token line saying what it "
          "has to say."),
        ("note", "**Two lines are the ones to sit with.** Tokens, which the agent cannot see "
                 "and the platform can, so every other line is a proxy for the bill. And the "
                 "five commands the harness blocked, each a decision handed to the deployer: "
                 "the fifth cost line from the walkthrough, another person's hour, appearing "
                 "in a real ledger for the first time and counted by the thing that spent "
                 "it."),

        ("h2", "The clauses that were in force before anybody wrote a cost policy"),
        ("p", "The session had a cost policy. It was spread across the harness's standing "
              "rules and the deployer's messages, and nobody had called it one. **Every "
              "clause in it is over a count, a place, a frequency or a delegation**: commit "
              "only when asked, no subagents, scratch files in the scratchpad, do not poll. "
              "Not one is a row in the mandate table on the deployment page, which is the "
              "finding the cost walkthrough predicted a day earlier and this case confirms "
              "with the site's own numbers."),
        s("v090-clauses-kept",
          "A table of six clauses that were in force during the session, where each came "
          "from, and whether it was kept",
          "Six clauses, two sources, and an honest third column: one kept mostly, one broken "
          "once and caught by the gate, one not kept at all."),
        ("p", "The kept column is the agent grading itself, and the case says so: **no "
              "accountant has read this ledger**, the status carries that sentence, and the "
              "gate refuses a case with a ledger whose status does not say one way or the "
              "other."),

        ("h2", "For once the grant is measured"),
        ("p", "Every other case on this site says its grant is not measured, and the gate "
              "insists on the words. This one runs on `anthropic/claude-code-remote/"
              "ccr-container`, 13 of 20 rows seen on the container itself, so the deployment "
              "is the published shape and the delta on the grant side is the shape's own. "
              "The gate allows the phrase the published shape only when the named shape has "
              "measured rows, and it then requires the delta not to be marked provisional."),
        s("v090-measured-delta",
          "The delta table on the session deployment page: the shape, 15 of 23 primitives, "
          "13 of 20 rows measured, the mandate, the excess and the unbounded excess",
          "The one delta on a case page that is not provisional. The mandate side is still "
          "a draft elicited from nine messages, and the page says which side is which."),

        ("h2", "The second case turns on one rule"),
        ("p", "A deployer who runs one assistant in the browser, as a coding agent and as a "
              "desktop work product, over one account. **The account holds the record of "
              "every past conversation, and the record contains secrets**, because things "
              "get pasted into a chat that would never be committed anywhere. So the record "
              "is a credential store, reading it is reading credentials, and the deployer's "
              "rule is that reading it should always be on demand."),
        figures.three_surfaces(),
        ("p", "The first case found that four grants over one Google account union into the "
              "account's exposure. This one adds a dimension: **the union runs in time as "
              "well as across surfaces.** Everything ever pasted is in the record, and "
              "turning reading off today does not take it out. A mandate over this estate "
              "has to say what to do about what is already there, which is why the case "
              "carries a prompt that finds the secrets so they can be removed, and says on "
              "its face that the prompt is itself a read of the record."),
        s("v090-web-mandate",
          "The mandate table on the browser deployment page: one wanted line, two refused, "
          "each with how we know and the fragment it came from",
          "The browser deployment's mandate. One line said, one said and one inferred, and "
          "twenty unstated, because which connectors are enabled is the first open question."),

        ("h2", "What matters, before what is forbidden"),
        ("p", "The deployer named the concept in the memo: what is being given to the agent "
              "is context on what is important and what is not. **A mandate is that list "
              "before it is a list of prohibitions.** So the case carries a what matters "
              "table beside the mandate, and every clause set on its three pages opens with "
              "it. It changed how the clauses read: they start with the record matters, the "
              "repository is the work, the container is disposable, and the prohibitions "
              "follow from those rather than standing alone."),
        s("v090-what-matters",
          "A table headed what matters, and what does not, with four rows: matters most, "
          "matters unknown, does not matter, must never be reused",
          "The importance list. It is the part of a mandate the grammar cannot hold and the "
          "part an agent most needs, and it is one table."),

        ("h2", "What this release did not settle"),
        ("ul", [
            "**The ledger is graded by the agent that produced it.** The accountant prompt "
            "exists and has not been run on this case; when it is, its report goes beside "
            "the ledger and the status changes.",
            "**Two numbers on the ledger are estimates and two are invisible.** Pushes to the "
            "working branch and fetches are from memory; tokens and the deployer's time are "
            "the platform's and the deployer's to supply.",
            "**Nothing in the second case is measured except the coding agent's shape.** "
            "Whether the browser and the desktop product read the whole record is the first "
            "of six open questions, and Prompt A asks each surface directly.",
            "**The desktop product has no shape.** Its page holds a mandate, clauses and a "
            "declared gap, and the next release gives that product a walkthrough of its own.",
            "**The record's secrets are still in it.** The prompt that finds them is on the "
            "page; running it is the deployer's, once, on demand.",
        ]),
        ("p", "[The session case](cases/session-001/index.html) &#183; "
              "[The three surfaces case](cases/estate-002/index.html) &#183; "
              "[The cost walkthrough](cost/index.html) &#183; " + vlink("v0.9.0")),
    ]


BODIES["a-ledger-and-a-record"] = _v090
...
def _v100(up):
    def s(name, alt, caption):
        return shot(up, name, alt, caption, "v0.10.0", CAPTURED_V080)

    return [
        ("h2", "The same four steps, on purpose"),
        ("p", "The deployer's brief for this one was short: the same sequence of events, the "
              "workflow always the same, for people who run the assistant on their desktop. "
              "Two sets of items: help them find out what is going on, then give the agent "
              "what it needs to decide better for itself. **So the section is the third "
              "walkthrough in exactly the shape of the first two**, and the differences are "
              "all in what a machine is rather than in how the pages work."),
        s("v100-desktop-hub",
          "The desktop walkthrough's hub at v0.10.0: the heading asking what an assistant on "
          "your own machine can reach and what on it matters",
          "The hub. The lead says the one thing that changes on a machine: host means your "
          "machine, and the agent cannot tell which part of it matters."),

        ("h2", "On a machine, the character of the shape is the switch"),
        ("p", "The published profile for a desktop application with local tools is derived "
              "and not measured, ten of twenty three primitives, none seen on an instance. "
              "**Four of its rows sit at a setting**: reading any file the account can reach, "
              "changing any file, running commands, and changing its own permission "
              "settings. Each is one switch away, and the switch belongs to the account "
              "running the application, which is you. The hub puts the shape's own grant "
              "table on the page so the reader sees the four half circles before reading a "
              "word about them."),
        s("v100-shape-table",
          "The desktop shape's grant table on the hub: ten rows, four of them at a setting, "
          "with the glyph, the undo class, the barrier and how each row is known",
          "The shape's leaflet on the hub. Two rows have no switch at all: the past "
          "conversations the application keeps and the credentials a home directory holds. "
          "If the past conversations contain secrets, those two rows are one row."),

        ("h2", "The map comes before the rules"),
        ("p", "The deployer named the concept in the memo for the estate case and it is the "
              "spine of this walkthrough: **what is being given to the agent is context on "
              "what is important and what is not.** A permission says what is possible; a "
              "rule says what is forbidden; neither says that the folder called clients is "
              "other people's or that the file with the token in it must never be opened. "
              "An agent with the map decides better on its own. One without it guesses, "
              "reasonably, which is how a client folder ends up summarised into a shared "
              "document."),
        s("v100-map-prompt",
          "Prompt 4 of the desktop walkthrough: draft a map of the machine in four groups, "
          "the work, not mine, credentials and the record, without opening anything new",
          "Prompt 4 of 10. The agent drafts the map from what it has already seen, is told "
          "not to open anything new to do it, and puts anything it is unsure of in not "
          "mine so the person moves it rather than the agent guessing."),
        ("note", "**The correction is the map, as the correction was the mandate in the "
                 "first walkthrough.** Every folder the person moves between the four groups "
                 "is a thing the agent would otherwise have decided about on its own. The "
                 "prompt is written so that the draft errs toward not mine, because an "
                 "agent that guesses a client folder is the work has already done the "
                 "damage by the time anybody notices."),

        ("h2", "Rules that open with the map, and say which part of it they follow from"),
        ("p", "Step three's long prompt asks for the document in an unusual order: the map "
              "first, in its four groups exactly as corrected, then the rules grouped by "
              "those same four, plus commands and instructions found in content, **with "
              "every rule saying which group of the map it follows from.** A rule with a "
              "reason attached is one the agent can extend to a case the rule did not name, "
              "and that is the whole difference between a document it decides against and "
              "one it guesses around."),
        s("v100-rules-prompt",
          "Prompt 8: the full rule set opening with the map, grouped by the work, not mine, "
          "credentials, the record, commands, and instructions found in content",
          "Prompt 8 of 10. The commands group and the instructions found in content group "
          "are the two a machine adds to what the mailbox walkthrough had: a shell as you, "
          "and a disk full of text other people wrote."),

        ("h2", "A switch you can flip is a setting, and on a managed machine it is not"),
        ("p", "The fourth page applies the enforcer test to a toggle. An application running "
              "as your user account can change its own settings, because you can, and it is "
              "you. **So a switch in the application is not a control on the application.** "
              "It is the third barrier kind, and the same toggle is a boundary on a managed "
              "machine where an administrator holds it and you cannot flip it back. Which "
              "one you have is a fact about the deployment, not the product, which is the "
              "sentence the home page has been making about a different setting since "
              "v0.1.0."),
        s("v100-switch-table",
          "The four barriers on a machine: nothing, expectation, setting and boundary, each "
          "with an example from a desktop deployment",
          "The barrier table for a machine. The setting row lists the four toggles; the "
          "boundary row lists what a person would actually have to build: a second account, "
          "a folder the account cannot read, a device policy, a sandbox."),

        ("h2", "Three walkthroughs, one nav entry"),
        ("p", "With the third walkthrough the top navigation reached eight entries and two "
              "rows. The three now sit under one entry, each with its subtitle, and every "
              "step page keeps its own table of the step before and after. Small, and the "
              "kind of thing a release should say it did."),
        s("v100-home-nav",
          "The home page chrome at v0.10.0 with the navigation on one row and a Walkthroughs "
          "entry",
          "The chrome after the fold. One row again, with the version badge reading v0.10.0."),

        ("h2", "What this release did not settle"),
        ("ul", [
            "**The shape is derived and the walkthrough says so on every page.** Zero of "
            "eleven rows were seen on an instance. A reader's first prompt produces the "
            "measured version for their machine, and nothing here does.",
            "**The map is the reader's to draw and the site never sees it.** There is no "
            "case yet with a desktop map in it; the estate case's desktop deployment is a "
            "declared gap, and a filled in map from a real machine would be the first row.",
            "**The record's secrets prompt is a read of the record.** The page says to run "
            "it once, on demand, and close the conversation. Nothing enforces that.",
            "**Whether a desktop application reads past conversations, and by default or on "
            "demand, is not measured.** It is the first open question on the estate case "
            "and it is the same question here.",
            "**The three walkthroughs share a shape and not a module.** Each is written by "
            "hand in the same four steps. A fourth would be the point at which the shape "
            "becomes a template, and it is not yet.",
        ]),
        ("p", "[The desktop walkthrough](desktop/index.html) &#183; "
              "[The three surfaces case](cases/estate-002/index.html) &#183; "
              "[The four barriers](model/barriers/index.html) &#183; " + vlink("v0.10.0")),
    ]


BODIES["context-on-what-matters"] = _v100
