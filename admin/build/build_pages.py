...
VERSION_LOG = [
    ("v0.11.0", "2026-09-22",
     ...
     {
       "summary":
         "On 19 September the agent operating a mailbox through the Gmail connector read its "
         "own thirty tool schemas, checked them against the live permission page, sent mail "
         "with no prompt, relabelled sixteen messages in nineteen unprompted writes, hit one "
         "refusal it could not explain, and wrote the four objects into an sgit vault in the "
         "connector's own vocabulary, naming the missing join to this grammar as a gap. This "
         "release is that join. The six vault files are held verbatim and hashed; a second "
         "variant of the Gmail shape is promoted from them with every row citing its line; "
         "the agent's inferred mandate is published beside the site's starting one so the "
         "ratchet the vault warns about is a number rather than a warning; and one page on "
         "the mailbox walkthrough shows what the prompts produce when somebody runs them.",
       "commit": None,
       "vault": "02n7bz55",
       "reconstructed": False,
       "changes": [
         "data/contributed/riskmandate/gmail-agent-02n7bz55/: GRANT.md, MANDATE.md, "
         "DELTA.md, AGENTS.md, the README and the version records, copied unchanged from "
         "vault 02n7bz55 at v0.4.0 and hashed in the manifest. The raw session debrief and "
         "the operator's runbook app stay in the vault and are cited by path.",
         "The manifest gains a vaults list beside shapes: the vault, its version and commit, "
         "the read key published on purpose, who wrote it, what was and was not copied, and "
         "the mapping module. promote_data promotes a vault entry through that module; the "
         "gate accepts a vault entry's own retrieval time, counts vault shapes, requires "
         "every named source to be among the hashed files, and requires the read key to be "
         "in PUBLISHED.",
         "anthropic/gmail-connector/measured-2026-09-19: five primitives, five of five rows "
         "measured, thirty tools with their permission level, five things it cannot reach "
         "with evidence, three contradictions of which one settles the earlier profile's, "
         "four open questions, and seven things the grammar has no word for. send.message."
         "world sits at nothing, because send_message runs with no prompt; the earlier "
         "profile's create.schedule.tenant row is absent, because no filter tool exists.",
         "inferred-from-one-session: the agent's own mandate, marked inferred and not "
         "elicited, wanting two primitives. The site's starting mandate for the earlier "
         "variant is extended to this one, so two deltas are stored against one grant. They "
         "differ by exactly send.message.world, which is the ratchet as a number.",
         "A Setting node the build derives by diffing the two variants: the per tool "
         "approval on send_message, moving one barrier between setting and nothing.",
         "gmail/measured/: the page. The thirty tools split by permission, the grant in the "
         "grammar, what changed against the profile read from the pages, two mandates "
         "against one grant, the vault's HARD and SOFT tags mapped onto the four barriers, "
         "three things the session found that no page had said, and what stays open.",
         "The hub, step one and step four of the mailbox walkthrough cite the measured "
         "deployment where it bears on them.",
       ],
       "basis": [
         "sgit vault 02n7bz55 at v0.4.0, commit obj-cas-imm-7ded8a06b473, cloned read only "
         "on 22 September 2026 with the read key the operator supplied.",
         "The earlier profile for the same shape, contributed by riskmandate.ai at v0.4.4, "
         "which the vault settles one contradiction of and does not replace.",
         "The house pattern of two variants of one product one setting apart, in use for the "
         "coding agent since v0.1.0.",
       ],
     }),
    ("v0.10.1", "2026-09-22",
    ...
    ("v0.9.0", "2026-09-22",
     ...
     {
       "summary":
         "The cost walkthrough shipped with the line that no case ran its prompts yet. The "
         "first case in this release is that case: the session that built releases v0.4.0 to "
         "v0.8.1, on the one shape this site holds whose grant was measured by the thing "
         "being profiled, with a ledger of what it spent counted from the repository and the "
         "code host's workflow log, every line saying which of those it came from, or that it "
         "is an estimate, or that the agent cannot see it. The second case is a deployer who "
         "runs one assistant in the browser, as a coding agent and as a desktop work product, "
         "over one account that holds the record of every past conversation, which contains "
         "secrets; the mandate turns on one rule, that reading the past is on demand, and the "
         "case carries the concept the deployer named: what is being given to the agent is "
         "context on what is important and what is not.",
       "commit": None,
       "vault": None,
       "reconstructed": False,
       "changes": [
         "The cases module holds several cases rather than one, each bringing its own "
         "estate figure, its own middle section and its own first prompt, with the "
         "deployments table, the mandate pages, the clauses and the discovery prompt "
         "generic across all of them.",
         "Case session-001: one deployment, the published shape this site is maintained "
         "from, so for once the grant is measured and the delta is not provisional. A ledger "
         "of seventeen lines, a table of the six clauses that were actually in force and "
         "whether each was kept, the outside list, and three things the session would not "
         "do again. No accountant has read it and the status says so.",
         "Case estate-002: three deployments over one account, the browser against the "
         "derived connectors shape, the coding agent against the measured container shape, "
         "and the desktop work product as a declared gap. A what matters table before the "
         "mandate, six open questions, and one prompt to run on all three surfaces asking "
         "whether each can read the past, by default or on demand.",
         "A ledger record type, abp/ledger/v1, and gate check 16 extended: a grant may say "
         "the published shape only when that shape has measured rows, the delta is then not "
         "provisional and must not be marked so, every ledger line names its source from a "
         "closed set or says cannot see with no number, and a case with a ledger says "
         "whether an accountant has read it.",
         "The three surfaces figure: one person, three surfaces, one account holding the "
         "record, with the two unmeasured arrows labelled with a question mark.",
         "A finding confirmed rather than predicted: every clause that actually governed the "
         "site's own session was over a count, a place, a frequency or a delegation, and not "
         "one of them is a row in the mandate table.",
       ],
       "basis": [
         "The repository's history from v0.3.0 to v0.8.1 and the code host's workflow log, "
         "read on 22 September 2026, for every counted line of the ledger.",
         "The deployer's voice memo of 22 September 2026, transcribed automatically, for the "
         "second case; every quoted fragment checked against it.",
         "The measured profile for the container shape, 13 of 20 rows seen on the thing "
         "itself on 5 September, which is what lets one case have a grant.",
       ],
     }),
    ("v0.8.1", "2026-09-22",
